...
public class Exercises {
	...
	public String[] list2Array(List<String> stringList) {
		String[] newArray1 = stringList.toArray(stringList.toArray(new String[0]));
		return newArray1;
	}
	...
	public List<String> no4LetterWords(String[] stringArray) {
		List <String> newList = new ArrayList<>(Arrays.asList(stringArray));
		newList.removeIf(no4List -> no4List.length() == 4);
		return newList;
	}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (Integer div: intArray) {
			doubles1.add(div / 2.0);
		}
		return doubles1;
	}
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		for (Integer oddsOnly : integerArray) {
			if (oddsOnly % 2 != 0){
				odds1.add(oddsOnly);
			}
		}
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		List<String> fBuzzList = new ArrayList<>();
		for (Integer fB: integerArray) {
			if (fB % 3 == 0 && fB % 5 == 0) {
				fBuzzList.add("FizzBuzz");
			} else if (fB % 3 == 0) {
				fBuzzList.add("Fizz");
			} else if (fB % 5 == 0) {
				fBuzzList.add("Buzz");
			} else {
				fBuzzList.add(fB.toString());
			}
		}
		return fBuzzList;
	}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> iLeave = new ArrayList<>();
		for (int i = 0; i < Math.max(listOne.size(), listTwo.size()); i ++){
			if (i < listOne.size()) {
				iLeave.add(listOne.get(i));
			}
			if (i < listTwo.size()) {
				iLeave.add(listTwo.get(i));
			}
		}
		return iLeave;
	}
